...
import pursefashion from "@/app/(routes)/components/pursefashion";
...
function MenDetails() {
  ...
  const filteredData: any = pursefashion.filter(
    (item) => item.id === parseInt(Array.isArray(data) ? data[0] : data)
  );
  return (
    <div>
      {filteredData.map(
        (item: {
          ...
          return (
            <div key={index} className="pt-28 flex lg:flex-row flex-col items-center justify-center sm:mx-10 mx-5 min-h-screen">
              ...
              <div className="sm:pl-10 pl-5 text-xl">
